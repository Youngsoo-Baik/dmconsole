...
const QCResults = () => {
    ...
    // API 호출을 통해 데이터 가져오기
    useEffect(() => {
        apiClient.get(`${apiUrl}/console/control-results`, {
            headers: {
            ...
            const fetchedRows = response.data.content.map(item => ({
                ...
                qc_material: item.qcMaterialName,
                qc_level: item.qcMaterialLevel,
    ...
    const columns = [
        ...
        { field: 'cat_lot', headerName: `${t('qc_result.column.cat_lot')}`, flex: 1, minWidth: 100, headerAlign: 'center', align: 'center' },
        { field: 'qc_material', headerName: `${t('qc_result.column.qc_material')}`, flex: 1, minWidth: 100, headerAlign: 'center', align: 'center' },
        { field: 'qc_level', headerName: `${t('qc_result.column.qc_level')}`, flex: 1, minWidth: 100, headerAlign: 'center', align: 'center' },
        { field: 'error_code', headerName: `${t('qc_result.column.error_code')}`, flex: 1, minWidth: 100, headerAlign: 'center', align: 'center' },
    ];
    ...
    const handleFilterSearch = (event, values = {}) => {
        ...
        const newFilteredRows = baseRows.filter((row) => {
            ...
            return (
                ...
                (!values.qc_lot || row.qc_lot.includes(values.qc_lot)) &&
                (!values.qc_level || row.qc_level.includes(values.qc_level)) &&
                        ...
                        <Box sx={{ mt: '10px', ml: '21px', display: 'flex', flexDirection: 'column', gap: 2 }}>
                            <Box sx={{ display: 'flex', gap: '24px' }}>
                                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                                    <Typography sx={{ color: '#002a70' }}>{t('qc_result.filter_search.device_model')}</Typography>
                                    <CustomSelect
                                        id="model"
                                        name="model"
                                        ...
                                        placeholder={t('qc_result.filter_search.device_model_placeholder')}
                                ...
                                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                                    <Typography sx={{ color: '#002a70' }}>{t('qc_result.filter_search.serial')}</Typography>
                                    <CustomTextField
                                        id="serial"
                                        name="serial"
                                        placeholder={t('qc_result.filter_search.serial_placeholder')}
                            ...
                            <Box sx={{ display: 'flex', gap: '24px' }}>
                                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                                    <Typography sx={{ color: '#002a70' }}>{t('qc_result.filter_search.cat_lot')}</Typography>
                                    <CustomTextField
                                        id="cat_lot"
                                        name="cat_lot"
                                        placeholder={t('qc_result.filter_search.cat_lot_placeholder')}
                                        // description="This will be device serial number"
                                        error={false}
                                        disabled={false}
                                ...
                                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                                    <Typography sx={{ color: '#002a70' }}>{t('qc_result.filter_search.qc_material_lot')}</Typography>
                                    <CustomTextField
                                        id="qc_lot"
                                        name="qc_lot"
                                        placeholder={t('qc_result.filter_search.qc_material_lot_placeholder')}
                                        // description="This will be device serial number"
                                        error={false}
                                        disabled={false}
                                        value={formik.values.qc_lot}
                                        onChange={formik.handleChange}
                                        onBlur={formik.handleBlur}
                                        // error={formik.touched.myTextField && Boolean(formik.errors.myTextField)}
                                        // helperText={formik.touched.myTextField && formik.errors.myTextField}
                                ...
                                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                                    <Typography sx={{ color: '#002a70' }}>{t('qc_result.filter_search.qc_material_level')}</Typography>
                                    <CustomTextField
                                        id="qc_level"
                                        name="qc_level"
                                        placeholder={t('qc_result.filter_search.qc_material_level_placeholder')}
                                        // description="This will be device serial number"
                                        error={false}
                                        disabled={false}
                                        value={formik.values.qc_level}
                                        onChange={formik.handleChange}
                                        onBlur={formik.handleBlur}
                                        // error={formik.touched.myTextField && Boolean(formik.errors.myTextField)}
                                        // helperText={formik.touched.myTextField && formik.errors.myTextField}
                                ...
                                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                                    <Typography sx={{ color: '#002a70' }}>{t('qc_result.filter_search.error_code')}</Typography>
                                    <CustomTextField
                                        id="error_code"
                                        name="error_code"
                                        placeholder={t('qc_result.filter_search.error_code_placeholder')}
                                        // description="This will be device serial number"
                                        error={false}
                                        disabled={false}
